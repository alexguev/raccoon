...
ADPoroMechanicsCoupling::validParams()
{
  ...
  //params.addRequiredParam<MaterialPropertyName>("porepressure_name", "porepressure calculated from p in Materials");
...
ADPoroMechanicsCoupling::ADPoroMechanicsCoupling(const InputParameters & parameters)
  : ADKernel(parameters),
    ...
    //_porepressure(getADMaterialProperty<Real>(getParam<MaterialPropertyName>("porepressure_name"))),
